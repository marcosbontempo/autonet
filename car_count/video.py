...
# Captura do video gravado
capture = cv.CaptureFromFile(sys.argv[1])

#Parametros do video capturado
...
print 'Informacoes do video de entrada:'
...
print 'Fourcc: %d' % (fourcc)

# Variaveis e Parametros
# minima distancia entre as linhas inciais e finais de deteccao de veiculos, com histerese
distMinimaExisteVeiculo = 100
distMinimaNaoExisteVeiculo = 150
...
#Cria os escritores de saida e salva o movimento

dirSaida=sys.argv[2]
images_dir=sys.argv[3]
veiculos_dir=sys.argv[4]
...
for frameInicial,frameFinal in sequencias:    
    ...
    for iframe in xrange(frameFinal):
        ...
        if iframe < nFramesBG:
            ...
            if iframe == nFramesBG:                
                ...
                cv.SaveImage('%s/background.jpg' % (images_dir), background)
        ...
        if ProcessaImagem:
            ...
            if iframe != frameInicial:
                ...
                if colunasIniciais and colunasFinais:
                    ...
                    # mostra uma linha na imagem de saida com a media do inicio e fim e o desvio padrao
                    ...
                    # marca a distancias entre as linhas...  
		    print("Distancia entre as linhas: "+str(distanciaLinhas))             

                    distanciasLinhas.append(distanciaLinhas)

                    if (distanciaLinhas > distMinimaExisteVeiculo
                          and  ((minI > 120) and (maxF < 450)) 
                        ):                  
                        if temVeiculo == False:
			    # Calcula distancia entre o ultimo carro e o atual
		            distanciaCarros = iframe - framesProximos
			    print("Distancia entre os carros: " + str(distanciaCarros) + " frames")
			    if distanciaCarros > 5:
				    #Guarda o ultimo frame em que um carro foi idenficado
				    framesProximos = iframe
		                    nVeiculos += 1
		                    #salva imagem de cada veiculo detectado.
				    print("local do arquivo: " + dirSaida + veiculos_dir + '/veiculo_%03d - frame_%d.jpg')
		                    cv.SaveImage(dirSaida + veiculos_dir + '/veiculo_%03d - frame_%d.jpg' % (nVeiculos,iframe), frame)                            
	...
	os.system("echo "+str(nVeiculos)+" > output/cont_veiculos")
